...
<c:if test="${empty param.sc_string || empty param.sc_type}">
    ...
    <sql:query var="rs" dataSource="jdbc/mydb" maxRows="${maxRow}" startRow="${start}">
        SELECT no, title, image, uploadtime FROM achieve ORDER BY no DESC
    </sql:query>
</c:if>
...
<c:if test="${!empty param.sc_string && !empty param.sc_type}">
    <sql:query var="cnt" dataSource="jdbc/mydb">
        SELECT count(*) AS cnt FROM achieve WHERE title LIKE ?
    ...
    <c:if test="${param.sc_type=='title'}">
        <sql:query var="rs" dataSource="jdbc/mydb" maxRows="${maxRow}" startRow="${start}">
            SELECT no, title, image, uploadtime FROM achieve WHERE title LIKE ? ORDER BY no DESC
            <sql:param> <c:out value="%${param.sc_string}%" /></sql:param>
        </sql:query>
    </c:if>
    ...
    <c:if test="${param.sc_type=='comment'}">
        <sql:query var="cnt" dataSource="jdbc/mydb">
            SELECT count(*) AS cnt FROM achieve WHERE comment LIKE ?
            <sql:param> <c:out value="%${param.sc_string}%" /></sql:param>
        </sql:query>
        ...
        <sql:query var="rs" dataSource="jdbc/mydb" maxRows="${maxRow}" startRow="${start}">
            SELECT no, title, image, uploadtime FROM achieve WHERE comment LIKE ? ORDER BY no DESC
            <sql:param><c:out value="%${param.sc_string}%" /></sql:param>
        </sql:query>
    </c:if>
    ...
    <c:if test="${param.sc_type=='writer'}">
        <sql:query var="cnt" dataSource="jdbc/mydb">
            SELECT count(*) AS cnt FROM achieve as n JOIN user as u ON u.id = n.id WHERE n.id = (SELECT id FROM user WHERE name LIKE ?)
            <sql:param> <c:out value="%${param.sc_string}%" /></sql:param>
        </sql:query>
        ...
        <sql:query var="rs" dataSource="jdbc/mydb" maxRows="${maxRow}" startRow="${start}">
            SELECT no, title, image, uploadtime FROM achieve as n JOIN user as u ON u.id = n.id WHERE n.id = (SELECT id FROM user WHERE name LIKE ?) ORDER BY no DESC
            <sql:param><c:out value="%${param.sc_string}%" /></sql:param>
        </sql:query>
    </c:if>
...
<div style="margin-left: 100px; margin-right: 100px;" id="content" class="content">
    <div>
        ...
        <div style="text-align: center;" class="sub7_tab_menu">
            ...
            <a href="./achieve.jsp" style="display: inline-block;" class="sub7_tab check">성과</a>
        </div>
        ...
        <div style="margin-left: 30px;">
            ...
            <c:forEach var="row" items="${rs.rows}">
                <div style="width: 33%; display: inline-block">
                    <a style="width: 300px; color: #000000" href="achieve_viewer.jsp?articleno=${row.no}">
                        <img onerror='this.src="../image/logo.png"'
                             style="border: 1px solid #9E9E9E; width: 400px; height: 250px; "
                             src="${row.image}">
                    </a>
                    <br>
                    <a style="color: #000000" href="achieve_viewer.jsp?articleno=${row.no}"><p style="margin-top: 10px; margin-right: 30%; text-align: center; font-size: 14pt">${row.title}</p></a>
                    <br>
                    <fmt:parseDate value="${row.uploadtime}" var="noticePostDate" pattern="yyyy-MM-dd HH:mm:ss.S"/>
                    <p style="margin-bottom: 50px; margin-right: 30%; text-align: center;"><fmt:formatDate value="${noticePostDate}" pattern="yyyy.MM.dd HH:mm:ss"/></p>
                </div>
            </c:forEach>
        ...
        <!-- 글 쓰기 버튼 영역 -->
        <div class="board-btn">
            <a href="./write_achieve.jsp">
                <button id="write"><i class="fa fa-pencil" aria-hidden="true">글쓰기</i></button>
            </a>
        </div>

        <div style="margin-top: 20px; margin-left: 48%; text-align: justify">
            <!-- 하단부 페이지 이동버튼 만들기 -->
            <c:set var="i" value="1" />
